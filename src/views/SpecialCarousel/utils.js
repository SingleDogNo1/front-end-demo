const fx = {
  ...
  move3: function(obj, json, fnEnd, fTime, sType) {
    ...
    setTimeout(function() {
      ...
      setTimeout(function() {
        if (typeof json == 'function') json.call(obj)
        else Utils.setStyle(obj, json)
      }, 0)
    }, 0)
  }
}
const Utils = {
  setStyle: function(obj, json) {
    if (obj.length) for (let i = 0; i < obj.length; i++) Utils.setStyle(obj[i], json)
    else {
      if (arguments.length == 2) for (let i in json) obj.style[i] = json[i]
      else obj.style[arguments[1]] = arguments[2]
    }
  },
