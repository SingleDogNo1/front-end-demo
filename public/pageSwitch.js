;(function(ROOT, struct) {
  ...
  each('X Y '.split(' '), function(name) {
    ...
    TRANSITION['scroll' + name] = function(cpage, cp, tpage, tp) {
      ...
      transform
        ...
        : (cpage.style[XY[prop]] = cp * 100 + '%')
      if (tpage) {
        transform
          ? (tpage.style[transform] = 'translate' + prop + '(' + tp * 100 + '%)' + fire3D)
          : (tpage.style[XY[prop]] = tp * 100 + '%')
      }
    }
    ...
    TRANSITION['slide' + name] = function() {
      TRANSITION['slideCoverReverse' + name].apply(this, arguments)
    }

    TRANSITION['flow' + name] = function() {
    ...
    TRANSITION['flip' + name] = function(cpage, cp, tpage, tp) {
      ...
      if (perspective) {
        ...
        cpage.style[transform] =
          'perspective(1000px) rotate' + prop + '(' + cp * 180 * fix + 'deg)' + fire3D
        if (tpage) {
          tpage.style[backfaceVisibility] = 'hidden'
          tpage.style[transform] =
            'perspective(1000px) rotate' + prop + '(' + tp * 180 * fix + 'deg)' + fire3D
        }
      } else TRANSITION['scroll' + name].apply(this, arguments)
    }
    ...
    TRANSITION['flip3d' + name] = (function() {
      var inited
      return function(cpage, cp, tpage) {
        var prop = name || ['X', 'Y'][1 - this.direction],
          fe = prop == 'X' ? -1 : 1,
          fix = fe * (cp < 0 ? 1 : -1),
        ...
        if (preserve3d) {
          ...
          cpage.parentNode.style[transform] =
            'translateZ(-' + zh + 'px) rotate' + prop + '(' + cp * 90 * fe + 'deg)'
          cpage.style[transform] = 'rotate' + prop + '(0) translateZ(' + zh + 'px)'
          if (tpage) {
            tpage.style[transform] =
              'rotate' + prop + '(' + fix * 90 + 'deg) translateZ(' + zh + 'px)'
          }
        } else TRANSITION['scroll' + name].apply(this, arguments)
      }
    })()
    ...
    TRANSITION['flipClock' + name] = (function() {
      ...
      return function(cpage, cp, tpage, tp) {
        ...
        if (perspective) {
          ...
          createWrap(
            cpage._clone || (cpage._clone = cpage.cloneNode(true)),
            this.container,
            prop,
            0.5
          )
          ...
          cpage._clone.parentNode.style[transform] =
            'perspective(1000px) rotate' + prop + '(' + n + 'deg)'

          if (tpage) {
            createWrap(tpage, this.container, prop, 0)
            createWrap(
              tpage._clone || (tpage._clone = tpage.cloneNode(true)),
              this.container,
              prop,
              0.5
            )
            ...
            tpage.parentNode.style[transform] =
              'perspective(1000px) rotate' + prop + '(' + m + 'deg)'
            tpage._clone.parentNode.style[transform] =
              'perspective(1000px) rotate' + prop + '(' + n + 'deg)'
          }
          ...
          if (0 == cp || tp == 0) {
            ...
            cpage.style.height = cpage.style.width = cpage.parentNode.style.height = cpage.parentNode.style.width =
              '100%'
    ...
    TRANSITION['zoom' + name] = function(cpage, cp, tpage) {
      var zIndex = Number(Math.abs(cp) < 0.5)
      if (transform) {
        cpage.style[transform] =
          'scale' + name + '(' + Math.abs(1 - Math.abs(cp) * 2) + ')' + fire3D
    ...
    TRANSITION['bomb' + name] = function(cpage, cp, tpage) {
      var zIndex = Number(Math.abs(cp) < 0.5),
    ...
    each(' Reverse In Out'.split(' '), function(type) {
      TRANSITION['scrollCover' + type + name] = function(cpage, cp, tpage, tp) {
        ...
        zIndex ? (cr = 20) : (tr = 20)
        transform
          ? (cpage.style[transform] = 'translate' + prop + '(' + cp * cr + '%)' + fire3D)
          : (cpage.style[XY[prop]] = cp * cr + '%')
        cpage.style.zIndex = 1 - zIndex
        if (tpage) {
          transform
            ? (tpage.style[transform] = 'translate' + prop + '(' + tp * tr + '%)' + fire3D)
            : (tpage.style[XY[prop]] = tp * tr + '%')
        ...
        if (transform) {
          ...
          if (tpage) {
            tpage.style[transform] =
              ...
              tp * zIndex * 100 +
              '%) scale(' +
              ((1 - Math.abs(zIndex ? 0 : tp)) * 0.2 + 0.8) +
              ')' +
              fire3D
            tpage.style.zIndex = zIndex
          }
      ...
      TRANSITION['flipCover' + type + name] = function(cpage, cp, tpage, tp) {
        ...
        if (perspective) {
          zIndex ? (cp = 0) : (tp = 0)
          cpage.style[transform] =
            'perspective(1000px) rotate' + prop + '(' + cp * -90 + 'deg)' + fire3D
          cpage.style.zIndex = 1 - zIndex
          if (tpage) {
            tpage.style[transform] =
              'perspective(1000px) rotate' + prop + '(' + tp * -90 + 'deg)' + fire3D
            tpage.style.zIndex = zIndex
          }
        } else TRANSITION['scroll' + name].apply(this, arguments)
      }
  ...
  function isArrayLike(elem) {
    ...
    return (
      ...
      tp != 'function' &&
      tp != 'string' &&
      (elem.length === 0 || (elem.length && (elem.nodeType == 1 || elem.length - 1 in elem)))
    )
  }
  ...
  struct.prototype = {
    ...
    init: function(config) {
      ...
      addListener(
        this.container,
        STARTEVENT.join(' ') + ' click' + (this.mousewheel ? ' mousewheel DOMMouseScroll' : ''),
        handler
      )
    ...
    initStyle: function(elem) {
      var style = elem.style,
        ret
      each(
        'position:absolute;top:0;left:0;width:100%;height:100%;display:none'.split(';'),
        function(css) {
          ret = css.split(':')
          style[ret[0]] = ret[1]
        }
    ...
    setTransition: function(transition) {
      this.events.update.splice(
        0,
        1,
        isFunction(transition) ? transition : TRANSITION[transition] || TRANSITION.slide
      )
      return this
    },
    ...
    handleEvent: function(oldEvent) {
      ...
      switch (ev.eventCode) {
        ...
        case 3:
          if (canDrag) {
            ...
            if (ev.length && (ev.eventCode == 1 || this.drag)) {
              ...
              if (isDrag) {
                if ((+new Date() - tm < 500 && Math.abs(offset) > 20) || Math.abs(percent) > 0.5) {
                  index += offset > 0 ? -1 : 1
                }
        ...
        case 5:
          ...
          if (
            this.isStatic() &&
            +new Date() - this.latestTime > Math.max(1000 - this.duration, 0)
          ) {
  ...
  this.container = typeof wrap == 'string' ? document.querySelector(wrap) : wrap
  this.init(config || {})
})
